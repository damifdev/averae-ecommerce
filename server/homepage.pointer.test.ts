...
async function pointerAudit(sectionTitle: string, port: number): Promise<PointerResult> {
  ...
  try {
    ...
    const after = await evaluate<{ opacity: string; label: string; cardHovered: boolean; hitTarget: string }>(`(() => {
      ...
      return { opacity: overlay ? getComputedStyle(overlay).opacity : '', label: overlay?.textContent?.trim() ?? '', cardHovered: Boolean(card?.matches(':hover')), hitTarget: hit ? hit.tagName.toLowerCase() + '.' + hit.className : '' };
    })()`);
    return { before: target.before, after: after.opacity, label: after.label, cardHovered: after.cardHovered, hitTarget: after.hitTarget };
  } finally {
    socket.close();
  }
}
...
describe('homepage product-image pointer overlays', () => {
  it('reveals VIEW PRODUCT on pointer hover in both requested sections', async () => {
    ...
    try {
      ...
      expect(newArrivals).toMatchObject({ before: '0', label: 'View product', cardHovered: true });
      expect(editorsPicks).toMatchObject({ before: '0', label: 'View product', cardHovered: true });
