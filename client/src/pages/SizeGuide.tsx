...
import { brand, localizedSizeConversions, type SizeMarket } from '@/lib/brand';
...
export default function SizeGuide() {
  return <div className="min-h-screen bg-[#F6F0E6] text-[#382820]"><SiteHeader /><main>
    ...
    <section className="container grid gap-12 py-14 md:py-20 lg:grid-cols-[1fr_.32fr]"><div className="space-y-14">
      ...
      <section aria-labelledby="market-heading"><p className="eyebrow text-[#B7654A]">Global fit</p><h2 id="market-heading" className="mt-2 font-display text-3xl">Choose your market</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#866F62]">Use these labels when comparing a familiar size with an Áveraẹ size. Product-specific charts take precedence when a style has its own fit data.</p><div className="mt-5 overflow-x-auto border border-[#D7C2A7] bg-[#FFFDF8]"><table className="w-full min-w-[560px] text-left text-sm"><caption className="sr-only">Localized apparel and footwear size conversions</caption><thead className="border-b border-[#D7C2A7] text-[10px] uppercase tracking-[.12em] text-[#866F62]"><tr><th className="px-4 py-4 font-normal">Market</th><th className="px-4 py-4 font-normal">XS / 36 EU</th><th className="px-4 py-4 font-normal">S / 37 EU</th><th className="px-4 py-4 font-normal">M / 38 EU</th><th className="px-4 py-4 font-normal">L / 39 EU</th></tr></thead><tbody>{(Object.keys(localizedSizeConversions) as SizeMarket[]).map(market => <tr key={market} className="border-b border-[#E6D8C8] last:border-0"><th className="px-4 py-4 font-medium">{market} · {localizedSizeConversions[market].label}</th><td className="px-4 py-4 text-[#866F62]">{localizedSizeConversions[market].apparel.XS} · shoe {localizedSizeConversions[market].footwear['36']}</td><td className="px-4 py-4 text-[#866F62]">{localizedSizeConversions[market].apparel.S} · shoe {localizedSizeConversions[market].footwear['37']}</td><td className="px-4 py-4 text-[#866F62]">{localizedSizeConversions[market].apparel.M} · shoe {localizedSizeConversions[market].footwear['38']}</td><td className="px-4 py-4 text-[#866F62]">{localizedSizeConversions[market].apparel.L} · shoe {localizedSizeConversions[market].footwear['39']}</td></tr>)}</tbody></table></div></section>
    ...
    </div><aside className="space-y-4 lg:sticky lg:top-8 lg:self-start"><div className="bg-[#382820] p-6 text-[#FFFDF8]"><p className="eyebrow text-[#D7C2A7]">Find your fit</p><h2 className="mt-3 font-display text-3xl">Shop with confidence.</h2><p className="mt-3 text-sm leading-7 text-[#D7C2A7]">Explore the collection, then use the sizing notes on each product page to make your choice.</p><div className="mt-6 flex flex-wrap gap-2"><Link href="/shop?audience=women" className="action-link-light inline-flex items-center gap-2 bg-[#FFFDF8] px-4 py-3 text-[10px] uppercase tracking-[.15em] text-[#382820]">SHOP WOMEN <ArrowRight size={14} /></Link><Link href="/shop?audience=men" className="inline-flex items-center gap-2 border border-[#FFFDF8] px-4 py-3 text-[10px] uppercase tracking-[.15em] text-[#FFFDF8] transition hover:bg-[#FFFDF8] hover:text-[#382820]">SHOP MEN</Link><Link href="/shop?audience=kids" className="inline-flex items-center gap-2 border border-[#FFFDF8] px-4 py-3 text-[10px] uppercase tracking-[.15em] text-[#FFFDF8] transition hover:bg-[#FFFDF8] hover:text-[#382820]">SHOP KIDS</Link></div></div><Link href="/returns" className="focus-ring flex items-center justify-between border border-[#D7C2A7] bg-[#FFFDF8]/60 p-4 text-[10px] uppercase tracking-[.13em] transition hover:border-[#B7654A] hover:text-[#B7654A]">VIEW RETURNS POLICY <ArrowRight size={14} /></Link><Link href="/faq" className="focus-ring flex items-center justify-between border border-[#D7C2A7] bg-[#FFFDF8]/60 p-4 text-[10px] uppercase tracking-[.13em] transition hover:border-[#B7654A] hover:text-[#B7654A]">VIEW FAQs <ArrowRight size={14} /></Link><Link href="/contact" className="focus-ring flex items-center justify-between border border-[#D7C2A7] bg-[#FFFDF8]/60 p-4 text-[10px] uppercase tracking-[.13em] transition hover:border-[#B7654A] hover:text-[#B7654A]">CONTACT US <ArrowRight size={14} /></Link></aside></section>
