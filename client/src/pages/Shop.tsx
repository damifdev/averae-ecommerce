  ...
  return <article className="group" data-testid={`product-card-${p.id}`}>
    <div className="relative aspect-[4/5] overflow-hidden bg-[#D7C2A7]">
      ...
      <div className="product-card-overlay absolute inset-x-3 bottom-3 grid gap-2">
        ...
        {requiresOptions ? <Link href={`/product/${p.id}`} className="product-card-action pressable block bg-[#382820] py-3 text-center text-[10px] uppercase tracking-[.15em] text-[#FFFDF8]">Select options</Link> : <button type="button" onClick={quickAdd} className="product-card-action pressable block w-full bg-[#382820] py-3 text-center text-[10px] uppercase tracking-[.15em] text-[#FFFDF8]">{added ? 'Added to bag' : 'Quick add'}</button>}
