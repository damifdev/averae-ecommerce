...
const filterDefaults = { size: 'All sizes', length: 'All lengths', texture: 'All textures', style: 'All styles', colour: 'All colours', condition: 'All conditions', price: 'All prices', brand: 'All brands', collection: 'All collections', availability: 'All availability', rating: 'All ratings', trend: 'All trend status' };
...
export default function Shop() {
  ...
  return <div><SiteHeader /><main className="container py-12 md:py-16">
    ...
    <div className="flex items-center justify-between gap-4 border-b border-[#D7C2A7] py-5"><div className="hidden flex-1 items-center gap-4 md:flex"><label className="flex max-w-xs items-center gap-2 border-b border-[#D7C2A7] pb-1 text-[10px] uppercase tracking-[.15em]"><Search size={14} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search products" className="w-full bg-transparent text-xs outline-none placeholder:text-[#866F62]" aria-label="Search products" /></label><div className="flex flex-1 justify-end gap-4"><span className="self-center text-[10px] uppercase tracking-[.15em] text-[#866F62]">Filters</span><label className="block text-[10px] uppercase tracking-[.14em] text-[#866F62]">Audience<select value={audience} onChange={event => { setAudience(event.target.value); setCategory('All'); }} className={selectStyles}><option>All</option>{audienceCategories.map(item => <option key={item.slug}>{item.label}</option>)}</select></label><label className="block text-[10px] uppercase tracking-[.14em] text-[#866F62]">Category<select value={category} onChange={event => setCategory(event.target.value)} className={selectStyles}><option>All</option>{productCategories.map(item => <option key={item.slug}>{item.label}</option>)}</select></label>{filterSelect('size', 'Size', options.sizes)}{isHair && <>{filterSelect('length', 'Length', [...categoryFilterOptions.length])}{filterSelect('texture', 'Texture', [...categoryFilterOptions.texture])}{filterSelect('style', 'Style', [...categoryFilterOptions.style])}</>}{filterSelect('colour', 'Colour', options.colours)}{isThrift && filterSelect('condition', 'Condition', [...categoryFilterOptions.condition])}{filterSelect('availability', 'Availability', ['In stock', 'Low stock', 'Out of stock'])}</div></div><button data-testid="mobile-filter-trigger" type="button" className="pressable flex items-center gap-2 text-[10px] uppercase tracking-[.15em] md:hidden" onClick={() => setFilterOpen(true)}><SlidersHorizontal size={15} /> Filter{activeChips.length ? ` · ${activeChips.length}` : ''}</button><label className="ml-auto flex items-center gap-2 text-[10px] uppercase tracking-[.15em]">Sort by<select value={sort} onChange={event => setSort(event.target.value as typeof sortOptions[number])} className="bg-transparent text-[10px] outline-none" aria-label="Sort products"><option disabled>Sort by</option>{sortOptions.map(option => <option key={option}>{option}</option>)}</select></label></div>
