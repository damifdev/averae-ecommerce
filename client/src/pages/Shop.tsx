...
import { audienceCategories, productCategories, products, formatPrice } from '@/lib/brand';
...
function trendStatus(product: typeof products[number]) {
...
function Card({ p }: { p: typeof products[number] }) {
  ...
  return <article className="group">
    <div className="relative aspect-[4/5] overflow-hidden bg-[#D7C2A7]">
      ...
      <div className="product-card-overlay absolute inset-x-3 bottom-3">{requiresOptions ? <Link href={`/product/${p.id}`} className="product-card-action pressable block bg-[#382820] py-3 text-center text-[10px] uppercase tracking-[.15em] text-[#FFFDF8]">Select options</Link> : <button type="button" onClick={quickAdd} className="product-card-action pressable block w-full bg-[#382820] py-3 text-center text-[10px] uppercase tracking-[.15em] text-[#FFFDF8]">{added ? 'Added to bag' : 'Quick add'}</button>}<span className="sr-only" aria-live="polite">{added ? `${p.name} added to bag` : ''}</span></div>
    </div>
    <div className="flex justify-between gap-3 pt-4"><div className="min-w-0"><Link href={`/product/${p.id}`} className="text-sm hover:underline">{p.name}</Link><p className="mt-1 text-xs text-[#866F62]">{p.category} · {p.color}</p><p className="mt-1 text-[10px] uppercase tracking-[.12em] text-[#B7654A]">{trendStatus(p)}</p></div><div className="text-right text-sm"><p>{formatPrice(p.price)}</p>{p.compareAt && <p className="mt-1 text-xs text-[#866F62] line-through">{formatPrice(p.compareAt)}</p>}</div></div>
  </article>;
}
...
export default function Shop() {
  ...
  const shown = useMemo(() => {
    ...
    list = list.filter(product => !query.trim() || `${product.name} ${product.category} ${product.collection} ${product.color} ${product.badge ?? ''}`.toLowerCase().includes(query.toLowerCase()));
    ...
    if (filters.brand !== 'All brands') list = list.filter(() => filters.brand === 'Áveraẹ');
    ...
    if (filters.rating !== 'All ratings') list = [];
  ...
  return <div><SiteHeader /><main className="container py-12 md:py-16">
    ...
    {activeChips.length > 0 && <div className="flex flex-wrap items-center gap-2 border-b border-[#D7C2A7] py-4"><span className="eyebrow mr-2 text-[#866F62]">Applied</span>{activeChips.map(chip => <button type="button" key={chip.key} onClick={() => removeChip(chip.key)} className="inline-flex items-center gap-2 border border-[#B7654A] px-3 py-2 text-[10px] uppercase tracking-[.12em] text-[#382820]">{chip.label}<X size={12} /></button>)}<button type="button" onClick={clearAll} className="ml-auto text-[10px] uppercase tracking-[.14em] underline underline-offset-4">Clear all</button></div>}
    <div className="flex items-center justify-between gap-4 border-b border-[#D7C2A7] py-5"><div className="hidden flex-1 items-center gap-4 md:flex"><label className="flex max-w-xs items-center gap-2 border-b border-[#D7C2A7] pb-1 text-[10px] uppercase tracking-[.15em]"><Search size={14} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search products" className="w-full bg-transparent text-xs outline-none placeholder:text-[#866F62]" aria-label="Search products" /></label><div className="flex flex-1 justify-end gap-4"><span className="self-center text-[10px] uppercase tracking-[.15em] text-[#866F62]">Filters</span><label className="block text-[10px] uppercase tracking-[.14em] text-[#866F62]">Audience<select value={audience} onChange={event => { setAudience(event.target.value); setCategory('All'); }} className={selectStyles}><option>All</option>{audienceCategories.map(item => <option key={item.slug}>{item.label}</option>)}</select></label><label className="block text-[10px] uppercase tracking-[.14em] text-[#866F62]">Category<select value={category} onChange={event => setCategory(event.target.value)} className={selectStyles}><option>All</option>{productCategories.map(item => <option key={item.slug}>{item.label}</option>)}</select></label>{filterSelect('size', 'Size', options.sizes)}{filterSelect('colour', 'Colour', options.colours)}{filterSelect('availability', 'Availability', ['In stock', 'Low stock', 'Out of stock'])}</div></div><button type="button" className="pressable flex items-center gap-2 text-[10px] uppercase tracking-[.15em] md:hidden" onClick={() => setFilterOpen(true)}><SlidersHorizontal size={15} /> Filter{activeChips.length ? ` · ${activeChips.length}` : ''}</button><label className="ml-auto flex items-center gap-2 text-[10px] uppercase tracking-[.15em]">Sort by<select value={sort} onChange={event => setSort(event.target.value as typeof sortOptions[number])} className="bg-transparent text-[10px] outline-none" aria-label="Sort products"><option disabled>Sort by</option>{sortOptions.map(option => <option key={option}>{option}</option>)}</select></label></div>
    <div className="hidden border-b border-[#D7C2A7] py-5 md:grid md:grid-cols-5 md:gap-5">{filterSelect('brand', 'Brand', ['Áveraẹ'])}{filterSelect('collection', 'Collection', options.collections)}{filterSelect('price', 'Price', ['Under ₦75,000', '₦75,000–₦125,000', 'Over ₦125,000'])}{filterSelect('rating', 'Rating', ['4 stars & up'])}{filterSelect('trend', 'Trend status', ['Trending', 'New', 'Popular', "Editor's Pick"])}</div>
    {filterOpen && <div className="fixed inset-0 z-[80] bg-[#382820]/45 md:hidden" onClick={event => { if (event.target === event.currentTarget) setFilterOpen(false); }}><aside className="ml-auto flex h-full w-[min(90vw,380px)] flex-col bg-[#FFFDF8] p-6 shadow-2xl" role="dialog" aria-modal="true" aria-label="Product filters"><div className="flex items-center justify-between border-b border-[#D7C2A7] pb-5"><div><p className="eyebrow text-[#866F62]">Refine the edit</p><h2 className="mt-2 font-display text-3xl">Filters</h2></div><button type="button" aria-label="Close filters" onClick={() => setFilterOpen(false)} className="focus-ring"><X size={20} /></button></div><div className="flex-1 space-y-5 overflow-y-auto py-6"><label className="block text-[10px] uppercase tracking-[.14em] text-[#866F62]">Audience<select value={audience} onChange={event => { setAudience(event.target.value); setCategory('All'); }} className={selectStyles}><option>All</option>{audienceCategories.map(item => <option key={item.slug}>{item.label}</option>)}</select></label><label className="block text-[10px] uppercase tracking-[.14em] text-[#866F62]">Category<select value={category} onChange={event => setCategory(event.target.value)} className={selectStyles}><option>All</option>{productCategories.map(item => <option key={item.slug}>{item.label}</option>)}</select></label>{filterSelect('size', 'Size', options.sizes)}{filterSelect('colour', 'Colour', options.colours)}{filterSelect('price', 'Price', ['Under ₦75,000', '₦75,000–₦125,000', 'Over ₦125,000'])}{filterSelect('brand', 'Brand', ['Áveraẹ'])}{filterSelect('collection', 'Collection', options.collections)}{filterSelect('availability', 'Availability', ['In stock', 'Low stock', 'Out of stock'])}{filterSelect('rating', 'Rating', ['4 stars & up'])}{filterSelect('trend', 'Trend status', ['Trending', 'New', 'Popular', "Editor's Pick"])}</div><div className="grid grid-cols-2 gap-3 border-t border-[#D7C2A7] pt-5"><button type="button" onClick={clearAll} className="border border-[#382820] py-3 text-[10px] uppercase tracking-[.14em]">Clear all</button><button type="button" onClick={() => setFilterOpen(false)} className="bg-[#382820] py-3 text-[10px] uppercase tracking-[.14em] text-[#FFFDF8]">View {shown.length} products</button></div></aside></div>}
    {shown.length === 0 ? <div className="py-24 text-center"><p className="eyebrow text-[#B7654A]">Nothing here yet</p><h2 className="mt-3 font-display text-4xl">Try another expression.</h2><p className="mt-4 text-sm text-[#866F62]">{filters.rating !== 'All ratings' ? 'There are no rated products in this catalog yet.' : 'Clear a filter or explore what’s trending.'}</p><button type="button" onClick={clearAll} className="mt-7 inline-flex border-b border-[#382820] pb-2 text-[10px] uppercase tracking-[.15em]">Clear all filters</button></div> : <div className="mt-8 grid gap-x-6 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">{shown.map(product => <Card key={product.id} p={product} />)}</div>}
  </main></div>;
}
