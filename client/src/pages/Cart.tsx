...
import { CART_UPDATED_EVENT, cartItemCount, getCart, removeFromCart, updateCartQuantity, type CartItem } from '@/lib/store';
...
export default function Cart() {
  ...
  return <div><SiteHeader /><main className="container py-12 md:py-20">
    ...
    </div><aside className="motion-surface h-fit bg-[#D7C2A7] p-7 md:p-9"><h2 className="font-display text-3xl">Summary</h2><div className="mt-7 space-y-4 border-b border-[#cfc4b6] pb-6 text-sm"><div className="flex justify-between"><span>Subtotal</span><span>{formatPrice(subtotal)}</span></div><div className="flex justify-between"><span>Delivery</span><span>Complimentary</span></div></div><div className="flex justify-between py-6 text-base"><span>Total</span><span>{formatPrice(subtotal)}</span></div><Link href="/checkout" className="pressable flex items-center justify-center gap-3 bg-[#382820] py-4 text-[10px] uppercase tracking-[.16em] text-[#FFFDF8]">Proceed to checkout <ArrowRight size={15} /></Link><p className="mt-5 text-center text-xs leading-5 text-[#6f675d]">Complimentary delivery on orders over ₦150,000</p></aside></div>}
  </main></div>;
}
