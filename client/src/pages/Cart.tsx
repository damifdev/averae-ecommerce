...
import { CART_UPDATED_EVENT, cartItemCount, clearCart, getCart, removeFromCart, restoreCartItem, updateCartQuantity, type CartItem } from '@/lib/store';
...
export default function Cart() {
  ...
  const removeLine = (item: CartItem) => {
    const next = removeFromCart(item.id, item.size, item.color);
    setItems(next);
    toast.success('Removed from your bag', {
      id: `removed-bag-${item.id}-${item.size || 'default'}-${item.color || 'default'}-${Date.now()}`,
      duration: 5000,
      position: 'bottom-center',
      action: {
        label: 'Undo',
        onClick: () => {
          setItems(restoreCartItem(item));
  ...
  </main><BackToTop /><ClearBagDialog open={clearDialogOpen} onOpenChange={setClearDialogOpen} onConfirm={clearBag} /></div>;
}
