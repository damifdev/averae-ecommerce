  ...
  return <section className="border-y border-[#D7C2A7] py-10" aria-labelledby={`look-${audience.toLowerCase()}-heading`}>
    <div className="grid gap-8 lg:grid-cols-[1.05fr_.95fr] lg:items-start">
      ...
      <div>
        ...
        <div className="mt-3 flex min-h-10 flex-wrap items-center gap-3" aria-live="polite"><p className="text-xs text-[#866F62]">{status}</p>{addedProduct && <Link href="/cart" data-testid="shop-look-view-cart" className="action-link-dark inline-flex items-center gap-2 border-b border-[#382820] pb-1 text-[10px] uppercase tracking-[.14em]">VIEW CART <ArrowRight size={13} /></Link>}</div>
      </div>
    </div>
  </section>;
}
