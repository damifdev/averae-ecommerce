...
import EditorialMiniCart from '@/components/EditorialMiniCart';
...
function ShopTheLook({ productIds, image, title, description, audience, hotspots }: { productIds: readonly number[]; image: string; title: string; description: string; audience: 'Women' | 'Men' | 'Kids' | 'Unisex'; hotspots: readonly { productId: number; x: number; y: number; label: string }[] }) {
  ...
  const saveProduct = (product: Product) => {
    ...
    const saved = next.includes(product.id);
    setStatus(saved ? `${product.name} saved.` : `${product.name} removed from saved products.`);
    toast[saved ? 'success' : 'message'](saved ? `${product.name} saved to your wishlist.` : `${product.name} removed from saved products.`, { duration: 2200, position: 'bottom-center' });
  };
...
export function Edit() {
  ...
  useEffect(() => {
    const requested = new URLSearchParams(location.split('?')[1] ?? '').get('category');
    const matched = editorialTaxonomy.find(category => category.toLowerCase().replaceAll(' ', '-') === requested?.toLowerCase());
