...
import { ArrowRight, Bookmark, Check, Share2, ShoppingBag } from 'lucide-react';
import SiteHeader from '@/components/SiteHeader';
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious, type CarouselApi } from '@/components/ui/carousel';
import BackToTop from '@/components/BackToTop';
import { Link } from 'wouter';
import { addToCart, getSavedArticles, SAVED_ARTICLES_UPDATED_EVENT, toggleSavedArticle } from '@/lib/store';
...
function ShopTheLook({ productIds, image, title, description, audience, hotspots }: { productIds: readonly number[]; image: string; title: string; description: string; audience: 'Women' | 'Men' | 'Kids' | 'Unisex'; hotspots: readonly { productId: number; x: number; y: number; label: string }[] }) {
  ...
  return <section className="border-y border-[#D7C2A7] py-10" aria-labelledby={`look-${audience.toLowerCase()}-heading`}>
    <div className="grid gap-8 lg:grid-cols-[1.05fr_.95fr] lg:items-start">
      <div className="relative aspect-[4/3] overflow-hidden bg-[#D7C2A7]"><img src={image} alt={`${audience} editorial look`} className="h-full w-full object-cover" /><span className="absolute left-4 top-4 bg-[#FFFDF8] px-3 py-2 text-[10px] uppercase tracking-[.16em]">THIS LOOK · {audience}</span>{hotspots.map(hotspot => <button key={hotspot.productId} type="button" data-testid={`look-hotspot-${audience.toLowerCase()}-${hotspot.productId}`} aria-label={`Shop ${hotspot.label}`} title={hotspot.label} onClick={() => scrollToAnchor(`look-product-${audience.toLowerCase()}-${hotspot.productId}`)} className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#FFFDF8] bg-[#382820] p-2 text-[#FFFDF8] shadow-md transition-transform hover:scale-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FFFDF8]" style={{ left: `${hotspot.x}%`, top: `${hotspot.y}%` }}><span className="block h-2 w-2 rounded-full bg-[#D7C2A7]" /></button>)}</div>
      <div>
