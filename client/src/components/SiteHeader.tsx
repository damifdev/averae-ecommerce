...
import { audienceCategories, brand, editorialEntries, formatPrice, productCategories, products } from '@/lib/brand';
...
function MobileSection({ title, links, onNavigate }: { title: string; links: { label: string; href: string }[]; onNavigate: () => void }) {
  return <details open className="border-b border-[#D7C2A7] py-4"><summary className="cursor-pointer list-none text-[10px] uppercase tracking-[.16em] text-[#866F62]">{title}</summary><div className="mt-3 space-y-1">{links.map(link => <MenuLink key={link.label} href={link.href} onClick={onNavigate}>{link.label}</MenuLink>)}</div></details>;
}
...
export default function SiteHeader() {
  ...
  const [cartIds, setCartIds] = useState<number[]>(() => getCart());
  ...
  useEffect(() => {
    ...
    const onStorage = () => { setCartIds(getCart()); setWishlistIds(getWishlist()); };
    ...
    return () => { window.removeEventListener('scroll', onScroll); window.removeEventListener('keydown', onKeyDown); window.removeEventListener('storage', onStorage); document.removeEventListener('pointerdown', onPointerDown); };
  }, []);

  useEffect(() => { setOpenMenu(null); setMobileOpen(false); setAccountOpen(false); hideDrawer(); }, [location]);
  useEffect(() => () => { if (drawerCloseTimerRef.current !== null) window.clearTimeout(drawerCloseTimerRef.current); }, []);

  const cartProducts = useMemo(() => cartIds.map(id => products.find(product => product.id === id)).filter(Boolean) as typeof products, [cartIds]);
  const wishlistProducts = useMemo(() => wishlistIds.map(id => products.find(product => product.id === id)).filter(Boolean) as typeof products, [wishlistIds]);
  const subtotal = cartProducts.reduce((sum, product) => sum + product.price, 0);
  const suggestions = useMemo(() => {
    const term = query.trim().toLowerCase();
    const matchingProducts = term ? products.filter(product => `${product.name} ${product.category} ${product.color}`.toLowerCase().includes(term)).slice(0, 3) : products.slice(0, 3);
    const matchingEdit = editorialEntries.filter(entry => `${entry.title} ${entry.description}`.toLowerCase().includes(term)).slice(0, 2);
    return { matchingProducts, matchingEdit };
  }, [query]);
  ...
  const menuLinks: Record<MenuKey, { label: string; href: string }[]> = {
    ...
    trends: [{ label: 'Trending Now', href: '/trends' }, { label: 'Trending Products', href: '/trends?view=products' }, { label: 'Trending Styles', href: '/trends?view=styles' }, { label: 'Trending Colours', href: '/trends?view=colours' }, { label: 'African Fashion', href: '/trends?view=african-fashion' }, { label: "Editor's Picks", href: '/edit' }, { label: 'EXPLORE ALL TRENDS', href: '/trends' }],
  ...
  return <>
    ...
    <header ref={headerRef} className={`sticky top-0 z-50 border-b border-[#D7C2A7] bg-[#FFFDF8]/95 backdrop-blur transition-[box-shadow] duration-200 ${scrolled ? 'shadow-[0_8px_24px_rgba(56,40,32,.08)]' : ''}`}>
      <div className={`container flex items-center justify-between gap-4 transition-[height] duration-200 ${scrolled ? 'h-16' : 'h-20'}`}>
        ...
        <div className="flex flex-1 items-center justify-end gap-3 sm:gap-4">
          ...
          <div className="relative hidden sm:block"><button type="button" aria-label="Account" aria-expanded={accountOpen} onClick={() => { setAccountOpen(!accountOpen); setOpenMenu(null); hideDrawer(); }} className="icon-action focus-ring group relative"><UserRound size={17} strokeWidth={1.3} /><span className="icon-tooltip">Account</span></button>{accountOpen && <div className="absolute right-0 top-10 z-50 w-48 border border-[#D7C2A7] bg-[#FFFDF8] p-4 shadow-lg"><p className="eyebrow text-[#866F62]">{user ? `Hello, ${user.name || 'there'}` : 'Your account'}</p><div className="mt-3 space-y-1"><MenuLink href="/account" onClick={() => setAccountOpen(false)}>My Account</MenuLink>{user ? <><MenuLink href="/account#orders" onClick={() => setAccountOpen(false)}>Orders</MenuLink><MenuLink href="/account#wishlist" onClick={() => setAccountOpen(false)}>Wishlist</MenuLink><MenuLink href="/account#addresses" onClick={() => setAccountOpen(false)}>Addresses</MenuLink><button type="button" onClick={() => { setAccountOpen(false); void logout(); }} className="flex w-full border-b border-[#D7C2A7]/70 py-2 text-left text-sm hover:text-[#B7654A]">Logout</button></> : <><button type="button" onClick={() => { setAccountOpen(false); startLogin(); }} className="flex w-full border-b border-[#D7C2A7]/70 py-2 text-left text-sm hover:text-[#B7654A]">Sign In</button><button type="button" onClick={() => { setAccountOpen(false); startLogin(); }} className="flex w-full border-b border-[#D7C2A7]/70 py-2 text-left text-sm hover:text-[#B7654A]">Create Account</button></>}</div></div>}</div>
          <button type="button" aria-label={`Wishlist${wishlistIds.length ? `, ${wishlistIds.length} saved` : ''}`} onClick={() => showDrawer('wishlist')} className="icon-action focus-ring group relative"><Heart size={17} strokeWidth={1.3} /><span className="icon-tooltip">Wishlist</span>{wishlistIds.length > 0 && <span className="absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#B7654A] px-1 text-[8px] text-[#FFFDF8]">{wishlistIds.length}</span>}</button>
          <button type="button" aria-label={`Bag${cartIds.length ? `, ${cartIds.length} item${cartIds.length === 1 ? '' : 's'}` : ''}`} onClick={() => { showDrawer('bag'); setOpenMenu(null); }} className="icon-action focus-ring group relative"><ShoppingBag size={17} strokeWidth={1.3} /><span className="icon-tooltip">Bag</span>{cartIds.length > 0 && <span className="absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#B7654A] px-1 text-[8px] text-[#FFFDF8]">{cartIds.length}</span>}</button>
        </div>
      </div>
    ...
    {mobileOpen && <div className="fixed inset-0 z-40 bg-[#FFFDF8] pt-24 lg:hidden"><div className="container h-full overflow-y-auto pb-28"><div className="flex items-center justify-between border-b border-[#D7C2A7] pb-4"><p className="eyebrow text-[#866F62]">Menu</p><button type="button" aria-label="Close menu" onClick={() => setMobileOpen(false)} className="focus-ring"><X size={18} /></button></div><MobileSection title="Shop" links={audienceLinks} onNavigate={closeAll} /><MobileSection title="Categories" links={categoryLinks} onNavigate={closeAll} /><MobileSection title="Discover" links={[...discoveryLinks, { label: 'African Fashion', href: '/trends?view=african-fashion' }]} onNavigate={closeAll} /><div className="mt-6 grid grid-cols-2 gap-3"><Link href="/account" onClick={closeAll} className="border border-[#D7C2A7] p-4 text-[10px] uppercase tracking-[.14em]">Account</Link><Link href="/account#wishlist" onClick={closeAll} className="border border-[#D7C2A7] p-4 text-[10px] uppercase tracking-[.14em]">Wishlist</Link></div></div></div>}
    {searchOpen && <div className="fixed inset-0 z-[70] bg-[#FFFDF8]/98 px-5 py-6 backdrop-blur-sm" onClick={event => { if (event.target === event.currentTarget) setSearchOpen(false); }}><div className="container"><div className="flex items-center justify-between border-b border-[#D7C2A7] pb-5"><p className="font-display text-3xl">Search Áveraẹ</p><button type="button" aria-label="Close search" onClick={() => setSearchOpen(false)} className="focus-ring"><X size={20} /></button></div><form onSubmit={event => { event.preventDefault(); submitSearch(); }} className="mt-8 flex items-center gap-3 border-b border-[#382820] pb-4"><Search size={19} strokeWidth={1.2} /><input autoFocus value={query} onChange={event => setQuery(event.target.value)} placeholder="Search products, brands, trends..." className="w-full bg-transparent font-display text-2xl outline-none placeholder:text-[#866F62]/70" aria-label="Search products, brands, trends" /></form><div className="mt-8 grid gap-8 md:grid-cols-3"><div><p className="eyebrow text-[#866F62]">Products</p><div className="mt-3 space-y-2">{suggestions.matchingProducts.map(product => <MenuLink key={product.id} href={`/product/${product.id}`} onClick={() => setSearchOpen(false)}>{product.name}</MenuLink>)}{query && !suggestions.matchingProducts.length && <p className="text-sm text-[#866F62]">No product matches yet.</p>}</div></div><div><p className="eyebrow text-[#866F62]">Categories & trends</p><div className="mt-3 space-y-2"><MenuLink href={`/shop?search=${encodeURIComponent(query || 'linen')}`} onClick={() => setSearchOpen(false)}>Linen Clothing</MenuLink><MenuLink href="/trends" onClick={() => setSearchOpen(false)}>Trending Now</MenuLink><MenuLink href="/trends?view=styles" onClick={() => setSearchOpen(false)}>Trending Styles</MenuLink></div></div><div><p className="eyebrow text-[#866F62]">The Edit</p><div className="mt-3 space-y-2">{suggestions.matchingEdit.map(entry => <MenuLink key={entry.slug} href={`/edit/${entry.slug}`} onClick={() => setSearchOpen(false)}>{entry.title}</MenuLink>)}{!suggestions.matchingEdit.length && <MenuLink href="/edit" onClick={() => setSearchOpen(false)}>Explore The Edit</MenuLink>}</div></div></div>{recentSearches.length > 0 && <div className="mt-10 border-t border-[#D7C2A7] pt-5"><p className="eyebrow text-[#866F62]">Recent searches</p><div className="mt-3 flex flex-wrap gap-2">{recentSearches.map(term => <button key={term} type="button" onClick={() => { setQuery(term); submitSearch(term); }} className="border border-[#D7C2A7] px-3 py-2 text-xs transition hover:border-[#B7654A] hover:text-[#B7654A]">{term}</button>)}</div></div>}</div></div>}
    {drawer === 'wishlist' && <div className={`drawer-backdrop z-[65] ${drawerVisible ? 'drawer-backdrop-open' : ''}`} role="presentation" onClick={() => hideDrawer()}><aside role="dialog" aria-modal="true" aria-label="Wishlist" onClick={event => event.stopPropagation()} className={`drawer-panel drawer-panel-right flex h-full w-full max-w-md flex-col bg-[#FFFDF8] p-6 shadow-2xl sm:p-8 ${drawerVisible ? 'drawer-panel-open' : ''}`}><div className="flex items-center justify-between border-b border-[#D7C2A7] pb-5"><div><p className="eyebrow text-[#866F62]">Saved pieces</p><h2 className="mt-2 font-display text-3xl">{wishlistProducts.length ? `${wishlistProducts.length} saved` : 'Your wishlist is empty'}</h2></div><button type="button" aria-label="Close wishlist" onClick={() => hideDrawer()} className="focus-ring"><X size={20} /></button></div><div className="flex-1 overflow-y-auto py-5">{wishlistProducts.length ? wishlistProducts.map(product => <div key={product.id} className="flex gap-4 border-b border-[#D7C2A7] py-4"><img src={product.image} alt={product.name} className="h-24 w-20 object-cover" /><div className="min-w-0 flex-1"><Link href={`/product/${product.id}`} onClick={() => hideDrawer()} className="text-sm hover:underline">{product.name}</Link><p className="mt-2 text-xs text-[#866F62]">{product.color}</p><p className="mt-3 text-sm">{formatPrice(product.price)}</p></div></div>) : <div className="py-12 text-center"><Heart className="mx-auto text-[#D7C2A7]" size={32} strokeWidth={1} /><p className="mt-5 font-display text-2xl">Keep discovering.</p><p className="mt-2 text-sm text-[#866F62]">Save pieces here while you explore the collection.</p></div>}</div><div className="border-t border-[#D7C2A7] pt-5"><Link href="/shop" onClick={() => hideDrawer()} className="block border border-[#382820] py-3 text-center text-[10px] uppercase tracking-[.14em]">Continue shopping</Link>{wishlistProducts.length > 0 && <Link href="/account#wishlist" onClick={() => hideDrawer()} className="mt-3 block border border-[#B7654A] py-3 text-center text-[10px] uppercase tracking-[.14em] text-[#B7654A]">View wishlist</Link>}</div></aside></div>}
    {drawer === 'bag' && <div className={`drawer-backdrop z-[65] ${drawerVisible ? 'drawer-backdrop-open' : ''}`} role="presentation" onClick={() => hideDrawer()}><aside role="dialog" aria-modal="true" aria-label="Shopping bag" onClick={event => event.stopPropagation()} className={`drawer-panel drawer-panel-right flex h-full w-full max-w-md flex-col bg-[#FFFDF8] p-6 shadow-2xl sm:p-8 ${drawerVisible ? 'drawer-panel-open' : ''}`}><div className="flex items-center justify-between border-b border-[#D7C2A7] pb-5"><div><p className="eyebrow text-[#866F62]">Your bag</p><h2 className="mt-2 font-display text-3xl">{cartProducts.length ? `${cartProducts.length} item${cartProducts.length === 1 ? '' : 's'}` : 'Your bag is empty'}</h2></div><button type="button" aria-label="Close bag" onClick={() => hideDrawer()} className="focus-ring"><X size={20} /></button></div><div className="flex-1 overflow-y-auto py-5">{cartProducts.length ? cartProducts.map(product => <div key={product.id} className="flex gap-4 border-b border-[#D7C2A7] py-4"><img src={product.image} alt={product.name} className="h-24 w-20 object-cover" /><div className="min-w-0 flex-1"><Link href={`/product/${product.id}`} onClick={() => hideDrawer()} className="text-sm hover:underline">{product.name}</Link><p className="mt-2 text-xs text-[#866F62]">{product.color}</p><p className="mt-2 text-xs text-[#866F62]">Quantity · 1</p><p className="mt-3 text-sm">{formatPrice(product.price)}</p></div></div>) : <div className="py-12 text-center"><ShoppingBag className="mx-auto text-[#D7C2A7]" size={32} strokeWidth={1} /><p className="mt-5 font-display text-2xl">Nothing here yet.</p><p className="mt-2 text-sm text-[#866F62]">Continue exploring to find your next piece.</p></div>}</div><div className="border-t border-[#D7C2A7] pt-5"><div className="flex justify-between text-sm"><span>Subtotal</span><span>{formatPrice(subtotal)}</span></div><div className="mt-4 grid grid-cols-2 gap-3"><Link href="/shop" onClick={() => hideDrawer()} className="border border-[#382820] py-3 text-center text-[10px] uppercase tracking-[.14em]">Continue shopping</Link><Link href="/cart" onClick={() => hideDrawer()} className="bg-[#382820] py-3 text-center text-[10px] uppercase tracking-[.14em] text-[#FFFDF8]">View bag</Link></div><Link href="/checkout" onClick={() => hideDrawer()} className="mt-3 block border border-[#B7654A] py-3 text-center text-[10px] uppercase tracking-[.14em] text-[#B7654A]">Checkout</Link></div></aside></div>}
    <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-[#D7C2A7] bg-[#FFFDF8]/95 py-2 backdrop-blur lg:hidden" aria-label="Mobile navigation"><Link href="/" aria-current={location === '/' ? 'page' : undefined} className={mobileNavClass('/')}><span>Home</span></Link><Link href="/shop" aria-current={location.startsWith('/shop') ? 'page' : undefined} className={mobileNavClass('/shop')}><span>Shop</span></Link><Link href="/trends" aria-current={location.startsWith('/trends') ? 'page' : undefined} className={mobileNavClass('/trends')}><span>Trends</span></Link><button type="button" aria-current={location.startsWith('/account') ? 'page' : undefined} onClick={() => showDrawer('wishlist')} className={`relative ${mobileNavClass('/account')}`}><span>Wishlist</span>{wishlistIds.length > 0 && <span className="absolute right-5 top-0 flex h-3 min-w-3 items-center justify-center rounded-full bg-[#B7654A] px-0.5 text-[7px] text-[#FFFDF8]">{wishlistIds.length}</span>}</button><button type="button" onClick={() => showDrawer('bag')} className={`relative ${mobileNavClass('/cart')}`}><span>Bag</span>{cartIds.length > 0 && <span className="absolute right-5 top-0 flex h-3 min-w-3 items-center justify-center rounded-full bg-[#B7654A] px-0.5 text-[7px] text-[#FFFDF8]">{cartIds.length}</span>}</button></nav>
  </>;
}
