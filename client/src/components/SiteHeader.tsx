...
const primaryMenuKeys: MenuKey[] = ['shop', 'trends', 'edit'];
...
const hairSubcategoryLinks = [
  { label: 'Human Hair', href: '/shop?category=hair&search=human%20hair' },
  { label: 'Blend Hair', href: '/shop?category=hair&search=blend%20hair' },
  { label: 'Packet Hair', href: '/shop?category=hair&search=packet%20hair' },
];
const thriftSubcategoryLinks = [
  { label: 'Women', href: '/shop?category=thrift-wear&audience=women' },
  { label: 'Men', href: '/shop?category=thrift-wear&audience=men' },
  { label: 'Kids', href: '/shop?category=thrift-wear&audience=kids' },
  { label: 'Vintage / Statement Pieces', href: '/shop?category=thrift-wear&search=vintage' },
];
...
const shopDiscoveryLinks = [{ label: 'VIEW ALL PRODUCTS', href: '/shop' }, ...discoveryLinks];
...
export default function SiteHeader() {
  ...
  const isActive = (key: MenuKey) => {
    ...
    if (key === 'hair') return location.startsWith('/hair') || location.includes('category=hair');
    if (key === 'thrift') return location.startsWith('/thrift-wear') || location.includes('category=thrift-wear');
    if (key === 'edit') return location.startsWith('/edit');
    if (key === 'shop') return location === '/shop' || location.startsWith('/shop?') || location.startsWith('/hair') || location.startsWith('/thrift-wear');
  ...
  const renderMegaMenu = (key: MenuKey) => {
    if (key === 'shop') return <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr_1fr_220px]"><MenuColumn title="Shop by audience" links={audienceLinks} onClick={closeAll} /><div><p className="eyebrow text-[#866F62]">Shop by category</p><div className="mt-4 space-y-1">{categoryLinks.map(link => <MenuLink key={link.label} href={link.href} onClick={closeAll}>{link.label}</MenuLink>)}<p className="pt-4 text-[9px] uppercase tracking-[.14em] text-[#866F62]">Hair</p>{hairSubcategoryLinks.map(link => <MenuLink key={link.href} href={link.href} onClick={closeAll}>{link.label}</MenuLink>)}<p className="pt-4 text-[9px] uppercase tracking-[.14em] text-[#866F62]">Thrift Wear</p>{thriftSubcategoryLinks.map(link => <MenuLink key={link.href} href={link.href} onClick={closeAll}>{link.label}</MenuLink>)}</div></div><MenuColumn title="Shop by discovery" links={shopDiscoveryLinks} onClick={closeAll} /><Link href="/edit" onClick={closeAll} className="group relative min-h-48 overflow-hidden bg-[#D7C2A7]"><img src={editorialEntries[0]?.image} alt="Featured editorial" className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-[#382820]/45" /><div className="absolute inset-x-4 bottom-4 text-[#FFFDF8]"><p className="eyebrow">Featured</p><p className="mt-2 font-display text-2xl">The Edit</p><span className="mt-3 inline-flex text-[9px] uppercase tracking-[.15em]">Explore story <ChevronRight size={13} /></span></div></Link></div>;
  ...
  return <>
    ...
    <header ref={headerRef} className={`relative z-50 border-b border-[#D7C2A7] bg-[#FFFDF8]/95 backdrop-blur transition-[box-shadow] duration-200 ${scrolled ? 'shadow-[0_8px_24px_rgba(56,40,32,.08)]' : ''}`}>
      <div className={`container flex items-center justify-between gap-4 transition-[height] duration-200 ${scrolled ? 'h-16' : 'h-20'}`}>
        ...
        <div className="absolute left-1/2 z-10 -translate-x-1/2"><Link href="/" aria-label="Áveraẹ home" className="font-display text-2xl tracking-[.16em]">{brand.name}</Link></div>
        <nav className="hidden flex-1 items-center justify-center gap-8 lg:flex" aria-label="Main navigation">{primaryMenuKeys.map(key => <div key={key} className="relative"><button type="button" aria-expanded={openMenu === key} aria-haspopup="true" onClick={() => { setOpenMenu(openMenu === key ? null : key); setAccountOpen(false); }} className={`nav-link flex items-center gap-1 text-[10px] uppercase tracking-[.16em] ${isActive(key) ? 'nav-link-active font-medium' : ''}`}>{menuLabels[key]}<ChevronDown size={11} className="opacity-50" /></button></div>)}</nav>
        <div className="flex flex-1 items-center justify-end gap-3 sm:gap-4">
          ...
          <div className="relative hidden sm:block"><button type="button" aria-label="Account" aria-expanded={accountOpen} onClick={() => { setAccountOpen(!accountOpen); setOpenMenu(null); hideDrawer(); }} className="icon-action focus-ring group relative"><UserRound size={17} strokeWidth={1.3} /><span className="icon-tooltip">Account</span></button>{accountOpen && <div className="absolute right-0 top-10 z-50 w-48 border border-[#D7C2A7] bg-[#FFFDF8] p-4 shadow-lg"><p className="eyebrow text-[#866F62]">{user ? `Hello, ${user.name || 'there'}` : 'Your account'}</p><div className="mt-3 space-y-1"><MenuLink href="/account" onClick={() => setAccountOpen(false)}>My Account</MenuLink><MenuLink href="/account#orders" onClick={() => setAccountOpen(false)}>Orders</MenuLink><MenuLink href="/wishlist" onClick={() => setAccountOpen(false)}>Wishlist</MenuLink><MenuLink href="/account#saved-addresses" onClick={() => setAccountOpen(false)}>Addresses</MenuLink>{user ? <button type="button" onClick={() => { setAccountOpen(false); void logout(); }} className="flex w-full border-b border-[#D7C2A7]/70 py-2 text-left text-sm hover:text-[#B7654A]">Logout</button> : <><button type="button" onClick={() => { setAccountOpen(false); startLogin(); }} className="flex w-full border-b border-[#D7C2A7]/70 py-2 text-left text-sm hover:text-[#B7654A]">Sign In</button><button type="button" onClick={() => { setAccountOpen(false); startLogin(); }} className="flex w-full border-b border-[#D7C2A7]/70 py-2 text-left text-sm hover:text-[#B7654A]">Create Account</button></>}<div className="mt-4 border-t border-[#D7C2A7] pt-3"><p className="eyebrow text-[#866F62]">Help &amp; Support</p><MenuLink href="/contact" onClick={() => setAccountOpen(false)}>Contact Us</MenuLink><MenuLink href="/faq" onClick={() => setAccountOpen(false)}>FAQs</MenuLink></div></div></div>}</div>
    ...
    {mobileOpen && <div className="fixed inset-0 z-40 bg-[#FFFDF8] pt-24 lg:hidden"><div className="mobile-drawer-panel container h-full overflow-y-auto pb-28"><div className="flex items-center justify-between border-b border-[#D7C2A7] pb-4"><p className="eyebrow text-[#866F62]">Menu</p><button type="button" aria-label="Close menu" onClick={() => setMobileOpen(false)} className="focus-ring"><X size={18} /></button></div><MobileSection title="Shop" links={[...audienceLinks, ...categoryLinks, ...hairSubcategoryLinks, ...thriftSubcategoryLinks, ...shopDiscoveryLinks]} onNavigate={closeAll} /><MobileSection title="Trends" links={menuLinks.trends} onNavigate={closeAll} /><MobileSection title="The Edit" links={menuLinks.edit} onNavigate={closeAll} /><MobileSection title="Help &amp; Support" links={helpSupportLinks} onNavigate={closeAll} /><div className="mt-6 grid grid-cols-2 gap-3"><Link href="/account" onClick={() => { trackEngagement('mobile_nav_click', { group: 'utility', label: 'account' }); closeAll(); }} className="border border-[#D7C2A7] p-4 text-[10px] uppercase tracking-[.14em]">Account</Link><Link href="/wishlist" onClick={() => { trackEngagement('mobile_nav_click', { group: 'utility', label: 'wishlist' }); closeAll(); }} className="border border-[#D7C2A7] p-4 text-[10px] uppercase tracking-[.14em]">Wishlist</Link></div></div></div>}
