...
import { cartItemCount, CART_UPDATED_EVENT, getCart, getWishlist, type CartItem } from '@/lib/store';
...
export default function SiteHeader() {
  ...
  useEffect(() => {
    ...
    const syncStore = () => { setCartItems(getCart()); setWishlistIds(getWishlist()); };
    ...
    <header ref={headerRef} className={`sticky top-0 z-50 border-b border-[#D7C2A7] bg-[#FFFDF8]/95 backdrop-blur transition-[box-shadow] duration-200 ${scrolled ? 'shadow-[0_8px_24px_rgba(56,40,32,.08)]' : ''}`}>
      <div className={`container flex items-center justify-between gap-4 transition-[height] duration-200 ${scrolled ? 'h-16' : 'h-20'}`}>
        ...
        <div className="flex flex-1 items-center justify-end gap-3 sm:gap-4">
          ...
          <button type="button" aria-label={`Bag${cartCount ? `, ${cartCount} item${cartCount === 1 ? '' : 's'}` : ''}`} onClick={() => { showDrawer('bag'); setOpenMenu(null); }} className="icon-action focus-ring group relative"><ShoppingBag size={17} strokeWidth={1.3} /><span className="icon-tooltip">Bag</span>{cartCount > 0 && <span className="absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#B7654A] px-1 text-[8px] text-[#FFFDF8]">{cartCount}</span>}</button>
        </div>
      </div>
