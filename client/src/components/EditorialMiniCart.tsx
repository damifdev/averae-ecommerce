...
export default function EditorialMiniCart({ open, onOpenChange, returnFocusRef }: EditorialMiniCartProps) {
  ...
  return <div className="fixed inset-0 z-[80]" role="presentation">
    <button type="button" aria-label="Close mini cart" onClick={close} className="absolute inset-0 bg-[#382820]/35" />
    <aside ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="editorial-mini-cart-title" data-testid="editorial-mini-cart" className="mini-cart-panel absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-[#FFFDF8] text-[#382820] shadow-2xl">
      <div className="flex items-center justify-between border-b border-[#D7C2A7] px-5 py-5 md:px-7"><div><p className="eyebrow text-[#B7654A]">Your selection</p><h2 id="editorial-mini-cart-title" className="mt-1 font-display text-3xl">Mini cart <span className="font-sans text-sm text-[#866F62]">({count})</span></h2></div><button ref={closeRef} type="button" aria-label="Close mini cart" onClick={close} className="focus-ring rounded-full p-2 hover:bg-[#F6F0E6]"><X size={19} strokeWidth={1.3} /></button></div>
